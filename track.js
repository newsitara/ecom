...
function renderOrderStatus(order) {
  ...
  if (itemsContainer) {
    itemsContainer.innerHTML = (order.items || [])
      ...
      <div class="order-item-card">
        ...
        <div class="item-price-tag">$${item.price * item.qty}.00</div>
      </div>
    `
      )
      .join('');
  ...
  document.getElementById('resShipMethod').textContent = order.shipping === 0 ? 'Complimentary Express (Free)' : `$${order.shipping}.00 Express`;
  document.getElementById('resGrandTotal').textContent = `$${order.total}.00 USD`;
